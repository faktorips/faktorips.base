...
public class StructuredSelectionHelper {
    ...
    public <T extends IIpsObject> T getFirstElementAsIpsObject(Class<T> clazz) {
        return switch (selection.getFirstElement()) {
            case IResource resource -> adaptToIpsObjectType(
                    IIpsModel.get().getIpsElement(Wrappers.wrap(resource).as(AResource.class)), clazz);
            case IAdaptable adaptable -> adaptToIpsObjectType(adaptable, clazz);
