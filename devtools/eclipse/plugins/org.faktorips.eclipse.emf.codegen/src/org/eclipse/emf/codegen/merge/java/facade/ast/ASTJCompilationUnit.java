...
public class ASTJCompilationUnit extends ASTJNode<CompilationUnit> implements JCompilationUnit {
    ...
    /**
     * Header of the compilation unit to be set during rewrite. If it is null, the header will not
     * be modified.
     */
     ...
     * <p>
     * Created for compatibility with JDOM implementation of <code>getName()</code> for
     * {@link JCompilationUnit}.
     * 
    ...
    /**
     * Returns the list of children in order: package declaration (<code>JPackage</code>), imports (
     * <code>JImport</code>), and types (<code>JType</code>).
     * 
    ...
    /**
     * This implementation remembers the header string, and does replacement of the header in the
     * final document after any other changes.
     * <p>
     * For the files with no package declaration, the new header might be inserted at the beginning
     * of the file instead of being replaced.
     * 
    ...
    public String getContents() {
        ...
        if (edits.getChildrenSize() != 0 || edits.getLength() != 0 || commenter != null || contentsReplacer != null
                || headerString != null) {
     ...
     * <p>
     * This implementation will not perform moving of the node if the node is inserted after being
     * removed. Hence, the removed node must not be inserted again.
     * 
    ...
    protected abstract class AbstractRewriter {
         ...
         * @param doc document after call to {@link ASTRewrite#rewriteAST()} or
         *            {@link ASTRewrite#rewriteAST(IDocument, Map)}.
    ...
    protected class NodeContentsReplacer extends AbstractRewriter {
        ...
        /**
         * Enables tracking for all nodes in tracked contents map (
         * {@link #getAllTrackedContentsMap()}).
         * <p>
         * This constructor must be called before call to {@link ASTRewrite#rewriteAST()} or
         * {@link ASTRewrite#rewriteAST(IDocument, Map)}.
         * 
        ...
        protected TextEdit addEdits(TextEdit existingEdits, IDocument doc) {
            for (Map.Entry<ITrackedNodePosition, String> entry : trackedNodePositionsMap.entrySet()) {
                ...
                try {
                    ReplaceEdit replaceEdit = new ReplaceEdit(position.getStartPosition(), position.getLength(),
                            contents);
    ...
    protected class NodeCommenter extends AbstractRewriter {
        ...
        protected TextEdit addEdits(TextEdit existingEdits, IDocument doc) {
            ...
            for (Map.Entry<ASTNode, ITrackedNodePosition> entry : commentedOutPositions.entrySet()) {
                ...
                try {
                    ...
                    // note that first line might move backward beyond node start (e.g. if a comma
                    // of the previous enum constant has been commented out)
                    ...
                    // if the node is less than 1 line long, but its contents is replaced by
                    // multiple lines,
                    ...
                    // reset text edits; since this node is processed successfully, there is no need
        ...
        /**
         * Add line break before the node if needed. If there was a line break added at this
         * position (e.g. after the previous node), replace it. Added line break will contain the
         * marker to comment out the first line of the node. If <code>InsertEdit</code> has been
         * created, the returned line number is the line after the line break, i.e. the next line
         * that has to be commented out. If no changes are made, returned line number is the first
         * line of the node.
         * 
         * @param existingEdits
         * @param nodePosition
         ...
         */
        protected int addLineBreakBeforeNode(TextEdit existingEdits, ITrackedNodePosition nodePosition, ASTNode node)
                throws BadLocationException {
            // insert line break at the first line if there is something before the node start and
        ...
        /**
         * Creates and adds <code>InsertEdit</code>s that comment out all lines between
         * <code>firstLine</code> and <code>lastLine</code> inclusively.
         * <p>
         * If there is a ReplaceEdit that covers positions where <code>InsertEdit</code>s are
         * inserted, then ReplaceEdit is replaced by another ReplaceEdit with modified text with all
         * lines commented out.
         * <p>
         * If there is any other problem adding new <code>InsertEdit</code>s to existing edits,
         ...
         * <p>
         * New ReplaceEdit has the same offset and length as the given ReplaceEdit. Text of new
         * ReplaceEdit has each line but the first one commented out. Given ReplaceEdit is removed
         * from its parent, and new ReplaceEdit is inserted in its place.
         * 
         * @param replaceEdit
        ...
        /**
         * Finds first ReplaceEdit in existing edits that covers node position range, and comments
         * out each line in it.
         * <p>
         * This method is used in the case when existing replace edit covers only a part of one
         * line, but the contents that it replaces is longer than 1 line. In this case, such replace
         * edit will be found and its contents changed by this method.
         * 
         * @param existingEdits
         * @param nodePosition range of existing node that has a corresponding ReplaceEdit for
         *            node's range
        ...
        protected void findAndCommentOutReplaceEdit(TextEdit existingEdits, ITrackedNodePosition nodePosition) {
            ...
            // this should be faster than lookup since underneath of addChild() binary search is
            // used
        ...
        /**
         * Comments out each line but the first one in the given text, and returns resulting new
         * text.
         * 
         * @param text
        ...
        /**
         * If there is anything after the node, inserts the line break to prevent commenting out
         * extra content.
         * 
         * @param existingEdits
         * @param nodePosition
         ...
         */
        protected void addLineBreakAfterNode(TextEdit existingEdits, ITrackedNodePosition nodePosition, ASTNode node)
                throws BadLocationException {
        ...
        protected class LineBreakInserter {
            ...
            /**
             * Creates line break at the beginning of the node when there is another node declared
             * at the same line. The returned edit also contains "//" after the line break - the
             * first line of the node becomes already commented out.
             * 
             * @param nodePosition
             * @param node
             * 
             ...
             * @throws BadLocationException
             */
            protected InsertEdit createLineBreakBeforeNode(ITrackedNodePosition nodePosition, ASTNode node)
                    throws BadLocationException {
                ...
                if (node.getNodeType() == ASTNode.ENUM_CONSTANT_DECLARATION) {
                    InsertEdit insertEdit = commentOutEnumConstantSeparator((EnumConstantDeclaration)node, lineInfo,
                            nodePosition);
                    if (insertEdit != null) {
                        return insertEdit;
                    }
                ...
                // if there is anything before the node on the same line, create line break and
                // comment out the first line of the node
            ...
            /**
             * Creates line break at the end of the node when there is another node declared at the
             * same line.
             * 
             * @param nodePosition
             * @param node
             ...
             * @throws BadLocationException
             */
            protected InsertEdit createLineBreakAfterNode(ITrackedNodePosition nodePosition, ASTNode node)
                    throws BadLocationException {
                ...
                // for enum constants, insert line break after the comma if there is anything after
                // the comma
                if (node.getNodeType() == ASTNode.ENUM_CONSTANT_DECLARATION) {
                    return createLineBreakAfterEnumConstant(node, lineInfo, nodePosition);
                }

                // if there is any content after the node on the same line, insert line break (to
                // prevent commenting out extra content)
             ...
             * <code>isCommentedOut</code> is <code>true</code>, and same indent string as the line
             * that break is inserted at.
             * 
             ...
             * @throws BadLocationException
             */
            protected String createLineBreakString(int startOfLineOffset, boolean isCommentedOut)
                    throws BadLocationException {
                return createLineBreakString(
                        getDocument().getLineDelimiter(getDocument().getLineOfOffset(startOfLineOffset)),
                        startOfLineOffset, isCommentedOut);
            }

            /**
             * Creates line break string containing line delimiter, line comment string if
             * <code>isCommentedOut</code> is <code>true</code>, and indent string that is the same
             * as of the line that break is inserted at.
             * 
             * @param lineDelimiter
             * @param startOfLineOffset
             * @param isCommentedOut
             ...
             * <p>
             * Separator of the previous constant needs to be commented out when all following
             * constants are commented out.
             * <p>
             * For example, if there are constants <code>C1, C2, C3;</code> and constants C2 and C3
             * are commented out, comma after C1 should be commented out as well. When this method
             * is called on the first line of C2, the returned edit will comment out a comma after
             * C1. Calling this method on C3 will return <code>null</code> since C2 and C3 are both
             * commented out.
             * <p>
             * Returned edit (if any) may or may not contain a line break depending whether there is
             * any content before the comma on the same line.
             * 
             * @param enumConstant
             * @param lineInfo
             * @param nodePosition
             ...
             * @throws BadLocationException
             */
            private InsertEdit commentOutEnumConstantSeparator(EnumConstantDeclaration enumConstant,
                    IRegion lineInfo,
                    ITrackedNodePosition nodePosition) throws BadLocationException {
                // if previous node is not commented out, but all the following nodes are, comment
                // out a comma (constant separator)
                ASTJNode<?> astjNode = (ASTJNode<?>)getFacadeHelper().convertToNode(enumConstant);
                if (astjNode != null) {
                    ASTNode parent = astjNode.getParent().getASTNode();
                    List<?> enumConstants = rewriter.getListRewrite(parent, EnumDeclaration.ENUM_CONSTANTS_PROPERTY)
                            .getRewrittenList();
                    ...
                    if (constantIndex > 0 && constantIndex < enumConstants.size()) {
                        ...
                        if (!commentedOutPositions.containsKey(previousNode)
                                && commentedOutPositions.keySet().containsAll(followingConstants)) {
                            ...
                            // we should be able to find the comma because the range for enum
                            // constants includes all preceding comments up to the previous constant
                            // if TargetSourceRangeComputer of ASTRewrite changes, this logic should
                            // change to skip comments
                            // see
                            // org.eclipse.emf.codegen.merge.java.facade.ast.CommentAwareSourceRangeComputer#getEnumConstantSourceRange(ASTNode)
                            if (commaPosition >= 0 && charContent[commaPosition] == ',') {
                                // if comma is on a line by itself, comment it out but do not insert
                                // the line break
             ...
             * @throws BadLocationException
             */
            private InsertEdit createLineBreakAfterEnumConstant(ASTNode node,
                    IRegion lineInfo,
                    ITrackedNodePosition nodePosition) throws BadLocationException {
                ...
                if (i < endOfLine) {
                    ...
                    //
                    // we should be able to find the comma because the range for enum constants
                    // includes all trailing comments up to the separator
                    // if TargetSourceRangeComputer of ASTRewrite changes, this logic should change
                    // to skip comments
                    // see
                    // org.eclipse.emf.codegen.merge.java.facade.ast.CommentAwareSourceRangeComputer#getEnumConstantSourceRange(ASTNode)
                    if (charContent[i] == ',') {
                ...
                // there is only whitespace after enum constant on the same line - do not add line
                // breaks
                return null;
            }
