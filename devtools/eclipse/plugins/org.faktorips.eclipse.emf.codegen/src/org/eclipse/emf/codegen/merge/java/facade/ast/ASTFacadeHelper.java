...
public class ASTFacadeHelper extends FacadeHelper {
    ...
    /**
     * Gets JavaCore options from JavaCore and updates tab and indentation settings from
     * ControlModel.
     * 
     * @return map of options
     * 
    ...
    private Map<String, String> getDefaultJavaCoreOptions() {
        ...
        if (DEBUG) {
            logInfo("Tab char: " + javaCoreOptions.get(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR)
                    ...
                    + ", Tab size: " + javaCoreOptions.get(DefaultCodeFormatterConstants.FORMATTER_INDENTATION_SIZE));
        }
     ...
     * Calls to <code>get...()</code> methods on the cloned node will not return the original
     * content. The returned node can only be inserted in the same tree that context node belongs
     * to.
     * 
     ...
     * <p>
     * If the source field has multiple variables declared in it, the returned field will contain
     * only 1 variable. Annotations, javadoc and initializer are replaced by original contents with
     * formatting preserved. Other parts of the declaration is not guaranteed to have formatting
     * preserved. The copied field will have source ranges for all nodes relative to the source
     * file, hence, get methods might return incorrect contents.
     * 
     * @param originalField
     * @param contextNode
     */
    ...
    public ASTJNode<?> findParent(ASTNode node) {
        JNode parent = null;
        // skip nodes in hierarchy that can not be converted to JNode (i.e.
    ...
    /**
     * Updates wrapped objects to nodes map. This method must be called when wrapped ASTNode of
     * ASTJNode is changed.
     * 
     * @param node
     */
    public void updateObjectToNodeMap(ASTJNode<?> node) {
        objectToNodeMap.put(node instanceof ASTJField ? ((ASTJField)node).getWrappedVariableDeclarationFragment()
                : node.getWrappedObject(), node);
    }
     ...
     * <p>
     * The given node must be one of the nodes of the AST tree created by
     * {@link #createCompilationUnit(String, String)}.
     * <p>
     * Note that contents is cached on the first access. Therefore, returned contents will not
     * reflect any changes to the nodes.
     * 
    ...
    public String toString(ASTNode node) {
        ...
        if (compilationUnit != null) {
            ...
            // String resultString = new String(originalContents,
            // compilationUnit.getASTCompilationUnit().getExtendedStartPosition(node),
            // compilationUnit.getASTCompilationUnit().getExtendedLength(node));
            ...
            if (rangeComputer instanceof CommentAwareSourceRangeComputer) {
                SourceRange sourceRange = ((CommentAwareSourceRangeComputer)rangeComputer)
                        .computeDefaultSourceRange(node);
