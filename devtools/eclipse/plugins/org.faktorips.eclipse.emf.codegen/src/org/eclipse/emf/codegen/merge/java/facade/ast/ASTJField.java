 ...
 * <code>ASTJField</code> for each variable. Each <code>ASTJField</code> has reference to
 * {@link FieldDeclaration} and {@link VariableDeclarationFragment}. Multiple <code>ASTJField</code>
 * can share the same {@link FieldDeclaration}.
 * <p>
 * If the field declaration referenced by <code>ASJField</code> has more than 1 variable, calls to
 * some <code>set...()</code> methods on the field and its children (e.g. annotations) will result
 * in a separation of the variable referenced by <code>ASTJField</code> from the field declaration.
 * The new declaration is inserted before the original declaration. After the separation, calls to
 * <code>get...()</code> methods will no longer return the correct original content.
 * 
 * @since 2.2.0
 */
public class ASTJField extends ASTJMember<FieldDeclaration> implements JField {
     ...
     * <p>
     * Note that change in initializer should not result in splitting variables in the field
     * declaration.
     * <p>
     * If <code>setInitializer()</code> has been called, and <code>performSplit()</code> is called
     * after, then <code>performSplit()</code> should not overwrite the initializer.
     */
    ...
    /**
     * Indicates whether the variable declaration fragment is the only fragment in the field
     * declaration.
     */
     ...
     * variable is <strong>not</strong> changed. On the other hand,
     * wrappedVariableDeclarationFragment is updated to reflect current node in the tree.
     * <p>
     * Since the same {@link FieldDeclaration} can have multiple variables declared in it, but
     * {@link JField} object must be unique for 1 variable, each ASTJField has reference to
     * FieldDeclaration and {@link VariableDeclarationFragment}.
     * 
    ...
    /**
     * Sets wrapped object to {@link VariableDeclarationFragment}, and prepares variable separation
     * if required
     * 
     ...
     * <p>
     * If the declaration has been split, <code>getComment()</code> might not return the correct
     * value.
     * 
    ...
    /**
     * May split the declaration same way as {@link ASTJField#setType(String)} If the declaration
     * has been split, <code>getFlags()</code> might not return the correct value.
     * 
    ...
    /**
     * Sets initializer of variable declaration fragment. This operation does not result in
     * splitting fields.
     * 
    ...
    public void setInitializer(String initializer) {
        ...
        setTrackedNodeProperty(variableDeclarationFragment, initializer,
                variableDeclarationFragment.getInitializerProperty(), ASTNode.INITIALIZER);
    }
    ...
    /**
     * Returns original type of {@link FieldDeclaration}. The dimensions declared after variable
     * name are appended to the type.
     * 
     ...
     * <p>
     * If there is only one {@link VariableDeclarationFragment} in this declaration, only the type
     * of the FieldDeclaration is changed.
     * <p>
     * If there are multiple {@link VariableDeclarationFragment}s in this declaration, variable
     * declaration fragment of this ASTJField is moved to a new {@link FieldDeclaration}, and the
     * type of new declaration is set.
     * <p>
     * Note that if field declaration has been split, <code>getType()</code>,
     * <code>getContents()</code>, <code>getComment()</code>, <code>getInitializer()</code> will not
     ...
     * <p>
     * If required, this method creates a new field declaration, and sets rewritten AST node to it.
     * The ASTNode of all annotations of the new field is updated to use the original node.
     * <p>
     * Note that the no changes are added to the rewriter or wrapped object until
     * {@link #performSplit()} is called.
     * <p>
     * This method must be called when field is created to ensure that annotations are unique for
     * each <code>ASTJField</code>.
     * 
     * @see #performSplit()
     */
     ...
     * object. If this declaration does not need to be split, reverts the changes made by
     * {@link #prepareSplit()}.
     * <p>
     * New field declaration will have only one variable declaration fragment. New declaration is
     * added to the {@link ASTRewrite}. The attributes of this ASTJField are updated to reference
     * elements of the new declaration. Only the javadoc, variable initializer, and annotations are
     * copied as String, all other attributes are copied using
     * {@link ASTNode#copySubtree(org.eclipse.jdt.core.dom.AST, ASTNode)}. All formatting except for
     * Javadoc, initializer, and annotations is lost. If field declaration wrapped by ASTJField has
     * only one variable declaration fragment left, no changes are made.
     * <p>
     * Note that this method must be called after {@link #prepareSplit()} and before any changes are
     * made to the nodes.
     * 
     * @see #prepareSplit()
     */
    ...
    /**
     * Sets wrapped object to be the given FieldDeclaration, and sets
     * wrappedVariableDeclarationFragment attribute to be the first variable declaration fragment in
    ...
    /**
     * Commenting out a field results in splitting of the fields, and then commenting out only the
     * field that is commented out.
     * 
