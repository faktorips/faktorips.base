...
public class XIndex extends AbstractGeneratorModelNode {
    ...
    private void createRangeKeyList() {
        for (IKeyItem keyItem : getIndex().getKeyItems()) {
            if (keyItem.isRange()) {
                rangeKeyNames.add(IpsStringUtils.toLowerFirstChar(getTableStructure().getRange(keyItem.getName())
                        .getParameterName()));
                rangeKeys.add(keyItem);
            } else if (getTableStructure().hasColumn(keyItem.getName())) {
                columnKeyNames.add(IpsStringUtils.toLowerFirstChar(keyItem.getName()));
    ...
    public String getColumnKeyNames() {
        return String.join(",", columnKeyNames);
    }
    ...
    public List<MethodParameter> getMethodParametersFindRow() {
        ...
        for (IKeyItem key : getIndex().getKeyItems()) {
            if (key.isRange()) {
                parameterName = IpsStringUtils
                        .toLowerFirstChar(getTableStructure().getRange(key.getName()).getParameterName());
            } else {
                parameterName = IpsStringUtils.toLowerFirstChar(key.getAccessParameterName());
            }
