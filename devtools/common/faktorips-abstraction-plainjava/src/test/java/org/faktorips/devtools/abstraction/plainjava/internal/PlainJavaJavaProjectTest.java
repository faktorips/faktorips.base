...
public class PlainJavaJavaProjectTest extends PlainJavaAbstractionTestSetup {
    ...
    public void testExists_not() {
        AProject project = Abstractions.getWorkspace().getRoot().getProject("NotSavedProject"); //$NON-NLS-1$
    ...
    public void testGetOutputLocation() {
        assertThat(javaProject.getOutputLocation(), is(Path.of("TestProject", "target"))); //$NON-NLS-1$ //$NON-NLS-2$
    }

    @Test
    public void testGetProject() {
        assertThat(javaProject.getProject(), is(notNullValue()));
        assertThat(javaProject.getProject(), is(sameInstance(newSimpleIpsProject("TestProject")))); //$NON-NLS-1$
    }

    @Test
    public void testIsJavaFolder() {
        AFolder srcFolder = javaProject.getProject().getFolder("src"); //$NON-NLS-1$
