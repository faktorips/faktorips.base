...
public class PlainJavaJavaElementTest extends PlainJavaAbstractionTestSetup {
    ...
    public void testGetResource_Packagefragment() {
        AFolder srcFolder = aProject.getFolder("src"); //$NON-NLS-1$
        APackageFragmentRoot pkgFragRoot = javaProject.toPackageFragmentRoot(srcFolder);

        assertThat(pkgFragRoot, is(wrapperOf(aProject.getLocation().resolve("src").toFile()))); //$NON-NLS-1$
    }
    ...
    public void testGetPath() {
        assertThat(javaProject.getPath(),
                is(Abstractions.getWorkspace().getRoot().getProject("TestProject").getWorkspaceRelativePath())); //$NON-NLS-1$
    }
}
