...
public class EnumAttributeModelTest {

    @Mock
    private EnumModel enumModel;

    private EnumAttributeModel fooModel;

    private EnumAttributeModel barModel;

    @Before
    public void initModels() throws SecurityException, NoSuchMethodException {
        Method fooGetter = MyEnum.class.getMethod("getFoo");
        fooModel = new EnumAttributeModel(enumModel, "foo", fooGetter);
        Method barGetter = MyEnum.class.getMethod("getBar", Locale.class);
        barModel = new EnumAttributeModel(enumModel, "bar", barGetter);
    }
