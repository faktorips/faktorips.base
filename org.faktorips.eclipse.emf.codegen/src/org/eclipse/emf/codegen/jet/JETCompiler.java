...
public class JETCompiler implements JETParseEventListener {
    ...
    protected Map<char[], JETConstantDataGenerator> constantDictionary = new HashMap<char[], JETConstantDataGenerator>(
            100, 100);

    protected long constantCount = 0;

    /**
     * If true, the newline immediately preceding a scriptlet or directive (though not a successful
     * include directive), along with any intervening spaces, will be stripped from the character
     * data.
     */
    ...
    /**
     * If fNoNewLineForScriptlets is true, the trailing newline/space sequence is stripped from each
     * character data segment, and stored in this field. Depending on what follows, it may then be
     * discarded or handled as its own character data segment.
     */
    protected char[] fSavedLine = null;

    /**
     * The depth of the current section, where 0 is outside of any sections. A section is delimited
     * by start and end directives, and must be preceded by an include directive with
     * fail="alternative".
     */
    protected int sectionDepth = 0;

    /**
     * Whether content is currently being skipped. This is set according to skipSections, as
     * sections are started and ended.
     */
    protected boolean skipping = false;

    /**
     * A stack of sections and whether to start skipping, one from each include with alternative
     * encountered.
     */
    ...
    public JETCompiler(String[] templateURIPath, String relativeTemplateURI, String encoding) throws JETException {
        ...
        this.reader = new JETReader(actualTemplateURI[1], relativeTemplateURI, openStream(actualTemplateURI[0]),
                encoding);
    }
        ...
        if (directive.equals("include")) {
            ...
            if (fileURI != null) {
                ...
                try {
                    ...
                    // processed as if it appeared in the included file.
                    //
                    if (fSavedLine != null) {
                        return;
                    }
            ...
            //
            SkipSection skipSection = skipSections.isEmpty() ? null : skipSections.peek();
            ...
            // This pop is safe because a section couldn't have been started without an include that
            // pushed.
            //
                ...
                if (skeletonURI != null) {
                    try {
                        BufferedInputStream bufferedInputStream = new BufferedInputStream(openStream(resolveLocation(
                                templateURIPath, templateURI, skeletonURI)[1]));
    ...
    public void endPageProcessing() throws JETException {
        ...
        if (skeleton == null) {
            throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.missing.jet.directive",
    ...
    public void parse() throws JETException {
        ...
        JETCoreElement[] coreElements = { directive, new JETParser.QuoteEscape(), new JETParser.Expression(),
                new JETParser.Scriptlet() };

        Class<?>[] accept = { JETParser.Directive.class, JETParser.QuoteEscape.class, JETParser.Expression.class,
