...
public class JETBuilder extends IncrementalProjectBuilder {
    ...
    protected void fullBuild(IProgressMonitor monitor) throws CoreException {
        ...
        if (nature != null) {
            JETCompileTemplateOperation compileTemplateOperation = new JETCompileTemplateOperation(getProject(),
                    nature.getTemplateContainers());
    ...
    protected void incrementalBuild(IResourceDelta delta, IProgressMonitor monitor) throws CoreException {
        ...
        if (nature != null) {
            if (delta.getKind() == IResourceDelta.ADDED || delta.getKind() == IResourceDelta.CHANGED
                    || delta.getKind() == IResourceDelta.CONTENT || delta.getKind() == IResourceDelta.OPEN) {
                JETCompileTemplateOperation compileTemplateOperation = new JETCompileTemplateOperation(getProject(),
                        nature.getTemplateContainers());
                compileTemplateOperation.setInBuild(true);
                if (compileTemplateOperation.shouldCompile()) {
