...
/**
 * JETReader is an input buffer for the JSP parser. It should allow unlimited lookahead and
 * push-back. It also has a bunch of parsing utility methods for understanding html-style things.
 */
public class JETReader {
     ...
     */
    public void stackStream(String baseURI, String locationURI, InputStream iStream, String encoding)
            throws JETException {
    ...
    /**
     * Register a new source file. This method is used to implement file inclusion. Each included
     * file gets a unique identifier (which is the index in the array of source files).
     * 
    ...
    /**
     * Register a new baseURI. This method is used to implement file inclusion. Each included file
    ...
    /**
     * Returns whether more input is available. If the end of the buffer for an included file is
     * reached, it will return to the context of the previous file, and return whether more input is
     * available from there. In this case, if trimExtraNewLine is true, then an unwanted extra
     * newline character will be suppressed. We consider the first newline in the buffer we are
     * returning to be unwanted if the ending buffer already has at least one trailing newline.
     */
    ...
    /**
     * Tests whether the current stream has at least one trailing newline, optionally followed by
     * spaces.
     */
    ...
    /**
     * Gets Content until the next potential JSP element. Because all elements begin with a '&lt;'
     * we can just move until we see the next one.
     */
    ...
    /**
     * Skip until the given string is matched in the stream. When returned, the context is
     * positioned past the end of the match.
     * 
     * @param limit The String to match.
     * @return A non-null <code>JETMark</code> instance if found, <strong>null</strong> otherwise.
     */
    ...
    /**
     * Parse a space delimited token. If quoted the token will consume all characters up to a
     * matching quote, otherwise, it consumes up to the first delimiter character.
     * 
    ...
    public String parseToken(boolean quoted) throws JETException {
        ...
        if (quoted) {
            if (ch == '"' || ch == '\'') {
                ...
                //
                if (ch == -1) {
                    throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.quotes.unterminated",
                            new Object[] { mark().toString() }));
                }
            } else {
                throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.attr.quoted",
                        new Object[] { mark().toString() }));
            }
        } else {
            if (!isDelimiter()) {
                ...
                do {
                    ...
                    if (ch == '\\') {
                        if (peekChar() == '"' || peekChar() == '\'' || peekChar() == endTagFinalChar
                                || peekChar() == endTagInitialChar) {
                            ch = nextChar();
                        }
                    }
                    stringBuffer.append((char)ch);
    ...
    /**
     * Parse an attribute/value pair, and store it in provided hash table. The attribute/value pair
     * is defined by:
     * 
     * <pre>
     ...
     * </pre>
     * 
     * Where <em>token</em> is defined by <code>parseToken</code> and <em>spaces</em> is defined by
     * <code>skipSpaces</code>. The name is always considered case insensitive, hence stored in its
     * lower case version.
     * 
    ...
    protected void parseAttributeValue(HashMap<String, String> into) throws JETException {
        ...
        skipSpaces();
        if (peekChar() != '=') {
            throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.attr.novalue",
                    new Object[] { name, mark().toString() }));
        }
        nextChar();
    ...
    /**
     * Parse some tag attributes for Beans. The stream is assumed to be positioned right after the
     * tag name. The syntax recognized is:
     * 
     * <pre>
     ...
     * 
     * @return A HashMap mapping String instances (variable names) into String instances (variable
     *         values).
     */
    public HashMap<String, String> parseTagAttributesBean() throws JETException {
        ...
        // Reached EOF:
        //
        throw new JETException(CodeGenPlugin.getPlugin().getString("jet.error.tag.attr.unterminated",
                new Object[] { mark().toString() }));
    }

    /**
     * Parse some tag attributes. The stream is assumed to be positioned right after the tag name.
     * The syntax recognized is:
     * 
     * <pre>
    ...
    /**
     * Parse utilities - Is current character a token delimiter ? Delimiters are currently defined
     * to be =, &gt;, &lt;, ", and ' or any any space character as defined by <code>isSpace</code>.
     * 
     * @return A boolean.
     */
