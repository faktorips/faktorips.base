 ...
 * <p>
 * Wrapped <code>ASTNode</code> (returned by {@link #getWrappedObject()}) is always the node in the
 * rewritten tree. Each object of <code>ASTJNode</code> wraps a unique <code>ASTNode</code>. After
 * nodes are removed, wrapped <code>ASTNode</code> has reference to the move target node that allows
 * the node to be inserted after, i.e. perform moving of the node.
 * <p>
 * Original ASTNode (returned by {@link #getASTNode()}) is used to make modifications to the node
 * and its children. This node must be used by get and set methods. Using this node allows to make
 * modifications to the nodes after they are moved.
 * <p>
 * Some subclasses (e.g. <code>ASTJAnnotation</code> and <code>ASTJField</code>) may need a
 * different <code>ASTNode</code> to be used by set methods. These subclasses use
 * <code>getASTNode()</code> only for get methods.
 * <p>
...
public abstract class ASTJNode<T extends ASTNode> extends AbstractJNode {
    /**
     * Default value used as default value of fields of subclasses. Indicates that fields have not
     * been initialized. This value should never be returned by any get...() methods.
     */
    ...
    /**
     * Flag that determines if parent property has been set (if not,
     * <code>ASTNode.getParent()</code> will be used to retrieve the parent).
     */
     ...
     * <p>
     * This attribute should used instead of ASTNode parent attribute. In some situations (i.e.
     * moving nodes by removing and inserting) ASTNode parent attribute might not be updated. In
     ...
     * <p>
     * This reference is used to notify the source range computer when the removed node is inserted
     * again (i.e. node is moved).
     */
     ...
     * <p>
     * When the node is removed, the wrapped node returned by {@link #getWrappedObject()} will be a
     * place-holder node, while this method will return the original node that can be modified by
    ...
    /**
     * Returns wrapped AST node that is currently in the rewritten tree, or to be inserted into the
     * tree. This node can be a move target node or any other temporary node. Methods such as
     * <code>set...</code> and <code>get...</code> should not use this node.
     * 
    ...
    /**
     * Sets the wrapped object to the given <code>ASTNode</code>. Must be used with caution and only
     * by subclasses.
     * 
     * @param node
     * @see #getWrappedObject()
    ...
    /**
     * Adds <code>value</code> as a new string place-holder to the <code>property</code> of the
     * <code>node</code>.
     * <p>
     ...
     * <p>
     * This implementation enables tracked contents for this node and calls itself on all of the
     * children.
     */
     ...
     * <p>
     * This implementation disables tracked contents for this node and calls itself on all of the
     * children.
     */
     ...
     * <p>
     * Parents that are interested in changes to children must override this method. Children that
     * need to notify the parents about changes have to call this method.
     * <p>
    ...
    /**
     * Temporarily disables tracking for all AST nodes that are tracked by this
     * <code>ASTJNode</code>.
     * <p>
     * Used when modified <code>ASTJNode</code>s are removed from the tree, and then possibly
     * inserted again.
     * 
    ...
    /**
     * Wraps up inserting new node. Notifies the children and the range computer that the node is
     * inserted.
     * 
     * @param newNode
     */
     ...
     * <p>
     * The contents of the node in AST implementation includes only the node itself without the
     * leading or trailing whitespace. If the node has a Javadoc comment, it is included in the
     * contents. No other leading or trailing comments are included in the node contents.
     * <p>
    ...
    /**
     * Inserts AST node wrapped by new node beside target node in the list defined by given
     * property.
     * <p>
    ...
    /**
     * Inserts AST node wrapped by new node at the beginning of the list of nodes defined by given
     * property.
     * <p>
     * No checks are performed if the new node can be inserted nor if the target node exists.
     * 
    ...
    /**
     * Inserts AST node wrapped by new node at the end of the list of nodes defined by given
     * property.
     * <p>
     * No checks are performed if the new node can be inserted nor if the target node exists.
     * 
    ...
    /**
     * Inserts <code>newSibling</code> as a child of this node before or after the given
     * <code>node</code>.
     * <p>
     * Default implementation does nothing and returns <code>false</code>.
     * 
     ...
     * @param before <code>true</code> if <code>newSibling</code> must be before <code>node</code>,
     *            <code>false</code> if after
     * @return <code>true</code> if operation successful, <code>false</code> otherwise
     */
     ...
     * <p>
     * This method notifies children and source range computer that the node is removed, then
     * creates a place-holder node that allows to insert the removed node later.
     * <p>
     ...
     * @param property
     */
    protected void removeNodeFromListProperty(ASTNode parentNode,
    ...
    /**
     * Sets the list property to the given array of values. For each string in the array, a string
     * place-holder node is created.
     * 
     * @param node
     * @param values
     * @param property
    ...
    /**
     * Sets the property of the node. The position of the property value is tracked and value is
     * replaced by <code>stringValue</code> during rewrite process.
     * 
     ...
     */
    protected void setTrackedNodeProperty(ASTNode node,
            String stringValue,
            StructuralPropertyDescriptor property,
            int nodeType) {
        if (ASTFacadeHelper.DEBUG) {
     ...
     * will replaced by the given <code>contents</code>. No changes will be made to
     * <code>contents</code>.
     * <p>
     * This method should be used to undo correction of indentation done by
     * {@link ASTRewrite#rewriteAST(org.eclipse.jface.text.IDocument, Map)} on string place-holder
     * nodes ({@link ASTRewrite#createStringPlaceholder(String, int)}) during rewrite process.
     * <p>
     * Note that all the tracked nodes must exist in the rewritten tree. If some tracked nodes are
     * removed from rewritten tree, wrong contents may be replaced. Use
     * {@link #disableTrackAndReplace()} when <code>ASTJNode</code>s are removed.
     * 
     * @param node
     * @param contents
     */
    ...
    /**
     * Comments out this node. Node is not removed from the tree, and is still returned by
     * getChildren().
     */
