...
def setVersionForPlatformBoms(String newVersion) {
    sh """
        for bom in \$(find devtools/common/bom/ -type d -name "*-platform-dependencies");
        do
            mvn -f \$bom/pom.xml versions:update-parent -DparentVersion=${newVersion} -DgenerateBackupPoms=false -DallowSnapshots=true -DskipResolution=true;
        done
    """
}
