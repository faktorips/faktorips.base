...
public interface IDeltaComputationOptions {
    ...
    public enum ComputationMethod{
        ...
        /**
         * Constant for the computation by position. See {{@link #getMethod(String)} for more details.
         */
        BY_POSITION,
    ...
    /**
     * Returns <code>true</code> if the two objects represent the same conceptual object, otherwise <code>false</code>.
     * What the conceptual object is, depends on the use case in that you need the delta computation and on your object model.
     * <p>
     * If the objects are for example contract versions, they represent the same contract, just at different
     * point in times. In this case the check has to compare the contract numbers but ignore the version numbers.
     * If the objects are coverages you might consider them the same, if they are of the same coverage type, e.g.
     * both are accidental damages coverages.
     */
