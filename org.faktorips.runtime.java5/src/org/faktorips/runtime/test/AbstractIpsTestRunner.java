...
public abstract class AbstractIpsTestRunner implements IpsTestListener {
     ...
     * <pre>{packageName in repository1}{packageName in repository2}{...}<pre>
     ...
     * The repository will be created by the to be implemented method: <code>createRepositories()<code>
     * @throws Exception if an error occurs.
     */
    ...
    /**
     * Sets additional packages, will be used to create the ClassloaderRuntimeRepository
     * which could be used in the runtime environment to obtain necessary objects.
     * Format: {package1}{package2}{...}{packagen}
     */
     ...
     * The format of the input string is:<br>
     * <pre>{repositoryName1}{repositoryName2}{...}<pre>
     */
    ...
    /*
     * Extract a list from the given string.
    ...
    /*
     * Pack as String from the given list.
     * Format: {elem1}{elem2}{...}{elem3}
     */
    ...
    /*
     * Returns the names without the first name.
     * format: in: {name1}{name2}{...}{namen}, out: {name2}{...}{namen}
     */
     ...
     * @params names contains the names in all repositories, 
     *               format: {<name in repository1>}{<name in repository2>}{<...>}...
     */
    private IpsTest2 getTest(String names) throws Exception {
        ...
        // create the ClassloaderRuntimeRepository repository which could be used in runtime to obtain
        // objects which are used in the test case
        // note: only one classloader repository will be created, it depends on the given ips project
        // this repository is the same for all test cases which itself are searched in different repositories (see below)
        ...
        for (int i = 0; i < createdRepositories.size(); i++) {
            ...
            ((IpsTestSuite)testSuite).addTest(createdRepositories.get(i).getIpsTest(getFirstTest(names), additionalRepositories));
