 ...
 * repository can access enum values without knowing whether they are stored in the Faktor-IPS
 * runtime repository or come from another data source.
 * 
 * @author Jan Ortmann
 * 
...
public interface IEnumValueLookupService<T> {
     ...
     * is contained only once. {@link IRuntimeRepository#getEnumValues(Class) will return the values
     * in the order defined by this list. The runtime repository does NOT cache the values, this is
     * the responsibility of the lookup service as it depends on the kind of data if caching is ok,
     ...
     * the default mechanism provieded by Faktor-IPS does not meet your requirements.
     * <p>
     ...
     * class implementing the typesafe enum pattern. This generated class has an annotation that
     * specifies that the generated XmlAdapter should be used by default for all attributes using
     * the enumeration type as data type. For example given the enumeration PaymentMode, Faktor-IPS
     * generates the adapter PaymentModeXmlAdapter and the JAXB annotation in the class PaymentMode
     * would be @XmlJavaTypeAdapter(PaymentModeXmlAdapter.class)
     * <p>
     ...
     * constructor. When unmarshaling the XML, you basically get the identifier stored in the XML
