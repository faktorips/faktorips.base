 ...
 * order among the keys in the map.
 * There are two different types of ranges this tree can support: one-column (unbound) ranges and two-column (bound) ranges.
 * The one-column tree interprets two consecutive keys as the bounds of a range. The tree
 ...
 * one side.
 * The two-column tree expects the keys of the given map to be instances of the static innner class TwoColumnKey. The upper and
 * lower bound of TwoColumnKeys are always inclusive, which means there are no different tree types for a two-column tree. There
 * is no check whether the keys of the map given to the two-column tree constructor are really instances of TwoColumnKey, instead
 * a ClassCastException may occur in getValue() in a two-column tree that was initialized with non-TwoColumnKey keys.
...
public class ReadOnlyBinaryRangeTree implements Serializable{
    ...
    /**
     * Constructor for a range tree. Keys of the provided map must be of type Comparable and really be comparable
     * (the compareTo()-method must return a defined value of -1, 0 or 1 on any combination of two keys in the map).
    ...
    /**
     * Visits the tree nodex starting from the startNode given to the start() method.
     * This visitor assumes that the keys of the visited nodes are instances of
     * TwoColumnKey, i.e. the given tree represents a set of two-column ranges.
     * 
     * @author Thorsten WÃ¤rtel
     */
