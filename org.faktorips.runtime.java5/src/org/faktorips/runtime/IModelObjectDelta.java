 ...
 * Projekt:     
 * Dateiname:   IModelObjectVersionDelta.java
 * Erzeugt:     24.04.2007
 *
 * Beschreibung:
 *
 ...
 * be instances of the same model class. One typical use case is that the two (technical) objects represent
 * the same business object but at two different points in time.
 * 
 * <p><strong>
...
public interface IModelObjectDelta {
     ...
     * <p>
     * If the object is a root object, this means that all properties are equal, 
     * no children have been removed, added or have changed their position. 
     * <p>
     * If this object is a dependant object (it belongs to a parent), this means
     * that all properties are equal and this object has neither been added to or
     * removed from its parent nor has the position of the object changed. 
     */
	public static final int EMPTY = 0;

    /**
	 * Delta kind constant indicating that the model object has been added 
	 * to it's parent. 
	 */
	public static final int ADDED = 16;
	
	/**
	 * Delta kind constant indicating that the model object has been removed
	 * from it's parent. 
	 */
	public static final int REMOVED = 8;

    /**
     * Delta kind constant indicating that the model object has been moved, meaning
     * it's position in the ordered list of objects has been changed. This type of delta
     * is only returned if the computation method is {@link IDeltaComputationOptions#COMPUTE_BY_OBJECT}.
     * <p>
     ...
     * If the object has changed, the type of change is further specified with
     * the following constants. Note that these constants are bit masks as more than one type of
     * change can occur.
     * <p>
     * Note that an object can be moved and changed! In this case getKind() returns MOVED & CHANGED.
     * 
    ...
    /**
     * Delta change type constant indicating that the object's structure has changed, 
     * that means either a child has been added, removed or 
     * changed its position (if the computation method is {@link IDeltaComputationOptions#COMPUTE_BY_OBJECT}
     * or a different object is now at a given position (if the computation method is {@link IDeltaComputationOptions#COMPUTE_BY_OBJECT}.
     */
	public static final int STRUCTURE_CHANGED = 1;
	
    /**
     * Delta change type constant indicating that at least one of the object's properties
     * has changed.
     */
	public static final int PROPERTY_CHANGED = 2;
	
    /**
     * Delta change type constant indicating that at least one child (that has neither
     * been added or removed or just moved) has changed. Either one of the child's properties has
     * changed or it's structure (or both). 
     */
	...
	/**
     * Returns the kind of change. Note that an object can be moved and changed! To allow this type of
     * information to be returned, a bit mask is used!
     * 
     * @see #EMPTY
	 * @see #ADDED
	 * @see #REMOVED
    ...
    /**
     * Returns <code>true</code> if the delta is empty, the two objects are the same, otherwise <code>false</code>.
     */
    public boolean isEmpty();

    /**
     * Returns <code>true</code> if this model object has been added to its parent, <code>false</code> otherwise.
     */
    public boolean isAdded();
    
    /**
     * Returns <code>true</code> if this model object has been removed from its parent, <code>false</code> otherwise.
     */
    public boolean isRemoved();
    
    /**
     * Returns <code>true</code> if this model object has changed its position in the ordered association.
     * This type of delta is only returned if the computation method is {@link IDeltaComputationOptions#COMPUTE_BY_OBJECT}.
     */
    public boolean isMoved();

    /**
     * Returns <code>true</code> if a different model object is at a position, otherwise <code>false</code>.
     * This type of delta is only returned if the computation method is {@link IDeltaComputationOptions#COMPUTE_BY_POSITION}.
     */
    ...
    /**
     * Returns <code>true</code> if this model object's structure has changed. That means either a child has
     * been added or removed or a child has been been moved.  
     */
     ...
     * then reference model object. Returns <code>false</code> otherwise. Returns <code>false</code> if propertyName
     * is <code>null</code>.
     */
     ...
     * Kind masks are formed by the bitwise or of <code>IModelObjectDelta</code> kind constants.
     * Returns an empty array if there are no affected children.
     */
