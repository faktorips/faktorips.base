...
public class AllClassesPageElement extends AbstractAllPageElement {
    ...
    public void build() {
        ...
        addPageElement(new TextPageElement(classes.size() + " Classes"));
        
        if (classes.size() > 0) {
            addPageElement(new ListPageElement(classes));
        }
    }
    ...
    protected List<PageElement> createClassesList() {
        ...
        for (IIpsObject object : objects) {
            ...
            PageElement link = new LinkPageElement(baseIpsElement, object, new TextPageElement(object.getName()), getLinkTarget());
            items.add(link);
        }
        return items;
    }
