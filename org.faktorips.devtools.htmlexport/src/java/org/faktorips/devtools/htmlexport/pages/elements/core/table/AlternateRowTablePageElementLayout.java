...
/**
 * The {@link AlternateRowTablePageElementLayout} layouts the even and uneven rows of a table with different {@link Style}s
 * @author dicker
 *
 */
public class AlternateRowTablePageElementLayout extends DefaultTablePageElementLayout {
	...
	public void layoutRow(int row, TableRowPageElement rowPageElement) {
		if (ignoreFirstRow && row == 0) return;
	...
	private Style getStyle(int row) {
		if (row % 2 == 0) return Style.TABLE_ROW_EVEN;
