...
public interface IRuntimeRepository {
    ...
    /**
     * Adds a repository this repository depends on because the one to add contains product data that is
     * referenced from this repository. Access methods like <code>getProductComponent(..)</code> include
     * all repositories this one depends on in their search.
     * 
    ...
    /**
     * Returns all repositories this one depends on directly or indirectly. The order is defined by a
     * breadth first search starting with this repository's direct dependencies. Each repository is only
     * included once even if it is referenced from two others. The list is computed lazy on the first
     * request.
     */
    public List<IRuntimeRepository> getAllReferencedRepositories();

    /**
     * Returns the product component identified by the given id. Returns <code>null</code> if the id is
     * <code>null</code> or no component with the indicated id can be found.
     * <p>
     * Note
     * <p>
     * How the product component id is structured, is defined as part of the ips project. The definition
     * is called a product component naming (and identification) strategy. The standard strategy is to
     * use the id of the product component kind followed by a separator followed by component's
     * versionId. However it is possible to use completly different strategies for example to create
     * numeric identifiers to reduce the size of indices in a datatbase. In the latter case the product
     * component id can't be derived from the product component kind id and it's version id.
     * 
    ...
    /**
     * Returns the product component identified by the given id. Same as getProductComponent(String id)
     * but throws an exception if the product component is not found. This method never returns null.
     * 
     * @param id The id of the product component to find.
     ...
     * <code>null</code> the most recent version is returned. Returns <code>null</code> if the kindId is
     * <code>null</code> or no component with the indicated kindId and versionId can be found.
     * 
    ...
    /**
     * Returns all product components that belong to the indicated product component kind. Returns an
     * empty array if either kindId is <code>null</code> or no component with the indicated kind is
     * found.
     * 
     * @param kindId The product component kind id, e.g. CollisionCoverage
     */
    ...
    /**
     * Returns the enumeration value for the provided enumeration class and string representation of the
     * value. If no value is found in the enumeration of the provided enumeration class
     * <code>null</code> will be returned. If the provided class cannot be recognized as a faktor ips
     * enumeration <code>null</code> will be returned. This method is only relevant for faktor ips
     * enumerations whose values are deferred to a content that is hold by this repository.
     * 
    ...
    /**
     * Returns the enumeration value for the provided unique Id. The unique Id is specified as follows
     * <i>qualifiedClassName'#'valueId</i>.
     * 
     ...
     * {@link IEnumValueLookupService#getEnumTypeClass()}. If a service is already registered for the
     * enumeration type, the new service replaces the old one.
     * 
     ...
     * <code>null</code> if either the id is <code>null</code>, the effectiveDate is <code>null</code>or
     * no generation with the indicated id can be found or the product component hasn't got a generation
     ...
     * getProductComponentGeneration(String id, Calendar effectiveDate) but throws an exception if the
     * product component generation is not found. This method never returns null.
     * 
     * @param id The product component's id.
     * @param effectiveDate The process' effective date
    ...
    /**
     * Returns all product components that are instances of the indicated class. Returns an empty list
    ...
    /**
     * Returns all product component generations for the given product component. Returns an empty list
     * if no generation is available.
     * <p>
     * The generations are ordered by valid from date in reverse order that means the latest generation
     * (latest valid from date) is the first one, the oldest generation is the last one.
     * 
     * @return The list of product component generations ordered by the valid from date in reverse order
     */
    ...
    /**
     * Returns the product component generation that follows the provided generation with respect to its
     * valid from date.
     * <p>
     ...
     * @throws IllegalArgumentException if the given product component generation could not be found in
     *             this repository or in any dependent repository.
     */
    public IProductComponentGeneration getNextProductComponentGeneration(IProductComponentGeneration generation);

    /**
     * Returns the product component generation that is prior to the provided generation with respect to
     * its valid from date.
     * <p>
    ...
    /**
     * Returns the test (either test case or suite) for the given qualified name. If a test is found for
     * the given qualified name, the test is returned. Otherwise a test suite containing all tests that
     * starts with the given qualified name is returned. Returns an empty test suite if no tests are
     * found for the given qualified name.
     * 
     ...
     * the given qualified name, the test is returned. Otherwise a test suite containing all tests that
     * starts with the given qualified name is returned. Returns an empty test suite if no tests are
     * found for the given qualified name. The given runtime repository is the repository which will be
     * used to instantiate the test cases.
     * 
    ...
    /**
     * Returns the test case for the given qualified name. The given runtimeRepository will be used to
     * instantiate the test case (this repository is used to search for test cases).<br>
     * Remark this runtime repository which will be used to search for the given test case can differ
     * from the runtime repository which will be used to instantiate the test case during runtime.<br>
    ...
    /**
     * Returns a test suite that contains all tests that have qualified names starting with the given
     * prefix. Note that if test cases belong to different package fragments the returned test suite
     * contains other testsuites. One suite for each package fragment.
     * 
     ...
     * Returns a test suite that contains all tests that have qualified names starting with the given
     * prefix. Note that if test cases belong to different package fragments the returned test suite
     * contains other test suites. One suite for each package fragment. The given runtime repository is
     * the repository which will be used to instantiate the test cases.
     * 
     * @see IRuntimeRepository#getIpsTestCase
     * 
     * @throws NullPointerException if qNamePrefix is <code>null</code>.
    ...
    /**
     * Returns a set containing the Java Class names of the implementation classes for all model types
     * available in this repository (either directly or via a referenced repository). For product
     * component types the implementation class for the part that remains unchanged over time is
     * returned. Currently there is no way to get get the implementation class for the product component
     * generation (the part that changed over time).
     * <p>
    ...
    /**
     * Creates a new JAXBContext that can marshall / unmarshall all modell classes defined in the given
     * repository. If the repository references other repositories (directly or indirectly), the context
     * can also handle the classes defined in these other repositories.
     */
    public JAXBContext newJAXBContext();

    /**
     * Getting a formula evaluator factory to create a new formula evaluator. If formula evaluation is
     * not supported, this method should return null.
     * 
    ...
    /**
     * Returns a object of type {@code T}, identified by it's qualified name, or {@code null} if no such
     * object exists in this repository or it's referenced repositories. It is up to extensions to
     * define which types can be found in the repository (e.g. a
     ...
     * {@link IRuntimeRepository} implementation does not allow custom types, {@code null} is returned.
     * 
     ...
     * {@link IRuntimeRepositoryLookup} is used for serialization of policy components especially for
     * the {@link ProductConfiguration} in configured policy components.
     * 
     * @return A previously set {@link IRuntimeRepositoryLookup} that is serialized by a
     *         {@link ProductConfiguration} and used to load the product component and its generation
