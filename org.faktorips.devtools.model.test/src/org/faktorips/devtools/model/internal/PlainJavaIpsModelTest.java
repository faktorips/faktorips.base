...
// TODO FIPS-8443 reactivate & move
@Ignore
public class PlainJavaIpsModelTest extends AbstractIpsPluginTest {

    private IpsModel model;

    // JavaProjects for testGetNonIpsResources()
    private PlainJavaProject javaProject = null;
    private PlainJavaProject javaProject2 = null;
    ...
    public void tearDown() throws Exception {
        IpsLog.setSuppressLoggingDuringTest(false);
        Abstractions.getWorkspace().getRoot().delete(null);
        getIpsModel().removeChangeListener(contentsChangeListener);
        tearDownExtension();
    }

    protected PlainJavaProject newPlainJavaPlatformProject(String name) {
    ...
    public void testCreateIpsProject() throws Exception {
        PlainJavaProject project = newPlainJavaPlatformProject("TestProject");
    ...
    @Test
    public void testGetIpsProjects() {
        newPlainJavaPlatformProject("TestProject");
        IIpsProject[] ipsProjects = model.getIpsProjects();
        assertEquals(0, ipsProjects.length);

        newIpsProject("TestPdProject");
        ipsProjects = model.getIpsProjects();
        assertEquals(1, ipsProjects.length);
        assertEquals("TestPdProject", ipsProjects[0].getName());

        newIpsProject("TestProject2");
        ipsProjects = model.getIpsProjects();
        assertEquals(2, ipsProjects.length);
        assertEquals("TestPdProject", ipsProjects[0].getName());
        assertEquals("TestProject2", ipsProjects[1].getName());
    }
    ...
    public void testGetNonIpsResources() {
        ICoreRunnable runnable = $ -> {
            javaProject = newPlainJavaPlatformProject("TestJavaProject");
            javaProject2 = newPlainJavaPlatformProject("TestJavaProject2");
        };
        AWorkspace workspace = Abstractions.getWorkspace();
        workspace.run(runnable, null);
    ...
    public void testClearIpsSrcFileContentsCacheWhenFileDeleted() throws Exception {
        ...
        // TODO: Wenn das File gelöscht wird muss das IpsModel das mitkriegen.
