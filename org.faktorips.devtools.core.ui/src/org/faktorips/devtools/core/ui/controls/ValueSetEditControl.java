 ...
 */
// NOTE: there is a constraint to faktor ips enum types with separate enum content that are used in
// a
...
public class ValueSetEditControl extends ControlComposite implements IDataChangeableReadWriteAccess {
    ...
    private Composite unspecifiedValueSetControl;
    ...
    /*
     * there is a contraint to faktor ips enum types with separate enum content that is used in a
     * modeling context in faktor ips. For those the possibility to restrict the values of the
     * datatype must not be possible. Thus only "AllValues" must be displayed in the
     * validTypesComboField.
     */
    private boolean usedForModelling;

    /**
     * Depicts if this {@link ValueSetEditControl} allows the definition of ValueSets with specific
     * values. If <code>true</code> allows Enums and Ranges with concrete values. If
     * <code>false</code> Valuesets are created without concrete values but with the abstract-Flag
     * set. Abstract valuesets must be specified on productside.
     * <p>
     * This flag can be changed by calling {@link #allowSpecifyValueSetValues(boolean)}.
     * 
     * @see IValueSet.isAbstract()
     */
    private boolean specifyValueSetValues;
     ...
     * 
     * @param specifyValueSetValues Sets the initial value for the flag
     *            {@link #specifyValueSetValues}. This value is designed to be changed at runtime
     *            (after the control has been displayed) by calling
     *            {@link #allowSpecifyValueSetValues(boolean)}
     */
    public ValueSetEditControl(Composite parent, UIToolkit toolkit, DefaultUIController uiController,
            IValueSetOwner owner, TableElementValidator tableElementValidator, boolean usedForModelling) {
        ...
        specifyValueSetValues = !owner.getValueSet().isAbstract();
            ...
            if (enumType != null) {
                if (!(enumControl instanceof EnumValueSetChooser)) {
                    groupComposite = createEnumValueSetGroup(valueSetArea,
                            Messages.ValueSetEditControl_labelAllowedValueSet);
                    ...
                    // because we reuse the old control and maybe the value set of the attribute has
                    // changed in the meanwhile
                    // (e.g. if we select all values then Attribute#setValueSetType is called and
                    // the internal enum values are deleted!)
                    ((EnumValueSetChooser)enumControl).setEnumTypeAndValueSet(enumType, (IEnumValueSet)valueSetOwner
                            .getValueSet());
                }
            } else {
                if (!(enumControl instanceof EnumValueSetEditControl)) {
                    ...
                    // (e.g. if we select all values then Attribute#setValueSetType is called and
                    // the internal enum values are deleted!)
                    ((EnumValueSetEditControl)enumControl).setEnumValueSet((IEnumValueSet)valueSetOwner.getValueSet());
                }
            }
    ...
    public void setTypes(ValueSetType[] valueSetTypes, ValueDatatype datatype) {
        ...
        // value set of enum types with separate enum contents cannot be restricted
    ...
    private class TypeModifyListener implements ValueChangeListener {
        ...
        public void valueChanged(FieldValueChangedEvent e) {
            ...
            updateValueSetType(newValueSetType);
        }

    }

    private void updateValueSetType(ValueSetType newValueSetType) {
        IValueSet oldValueSet = valueSetOwner.getValueSet();
        if (newValueSetType == ValueSetType.RANGE) {
    ...
    public void setEnabled(boolean enable) {
        setEnabledIfExistent(typLabel, enable);
        setEnabledIfExistent(validTypesCombo, enable);
        // FIXME RangeEditControl#setEnabled() sauber implementieren
        setEnabledIfExistent(rangeControl, enable);
        setEnabledIfExistent(enumControl, enable);
        setEnabledIfExistent(allValuesControl, enable);
    }
    ...
    public void allowSpecifyValueSetValues(boolean specifyValueSetValues) {
        this.specifyValueSetValues = specifyValueSetValues;
        // changes the valueset and Updates the UI
        updateValueSetType(valueSetOwner.getValueSet().getValueSetType());
    }

    public boolean isSpecifyValueSetValues() {
        return specifyValueSetValues;
    }
