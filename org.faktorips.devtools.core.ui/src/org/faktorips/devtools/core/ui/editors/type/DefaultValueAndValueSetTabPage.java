...
public class DefaultValueAndValueSetTabPage extends Composite {
    ...
    public DefaultValueAndValueSetTabPage(TabFolder folder, IValueSetOwner owner, IpsObjectUIController uiController,
            UIToolkit uiToolkit) {
        ...
        labelDefaultValue = uiToolkit.createFormLabel(workArea,
                Messages.DefaultValueAndValueSetTabPage_labelDefaultValue);
        ...
        /*
         * TODO pk the last two parameters of this constructor is set to true until the attribute
         * contraints concept is introduced to faktor ips. see the remark in the ValueSetEditControl
         * class
         */
    ...
    private void createDefaultValueEditField() {
        ...
        defaultValueField = datatypeCtrlFactory.createEditField(uiToolkit, defaultEditFieldPlaceholder, datatype, null,
                valueSetOwner.getValueSet().getIpsProject());
    ...
    private void updateValueSetTypes() {
        ...
        try {
            ...
            if (datatype != null) {
                ValueSetType[] types = ((IIpsObjectPartContainer)valueSetOwner).getIpsProject().getValueSetTypes(
                        datatype);
                valueSetEditControl.setTypes(types, datatype);
                if (prevSelectedType != null) {
                    // if the previous selction was a valid selection use this one as new selection
                    // in drop down,
    ...
    private class Validator implements TableElementValidator {
        ...
        public MessageList validate(String element) {
            MessageList list;
            try {
                list = ((IpsObjectPartContainer)valueSetOwner).validate(((IpsObjectPartContainer)valueSetOwner)
                        .getIpsProject());
