...
public abstract class ViewerButtonComposite extends Composite {
    ...
    /**
     * Creates the compoiste's controls. This method has to to be called explicitly
     * by subclasses <b>after</b> they have initialized any subclass specific
     * instance variables. The <code>ViewerButtonComposite</code> does not call this
	...
	/**
	 * Updates the button's enabled state depending on the object selected
	 * in the viewer.
	 */
	protected abstract void updateButtonEnabledStates();
	
	/**
	 * Returns the object that is selected in the viewer or <code>null</code>
    ...
    public void refresh() {
		if (viewer.getInput()==null) {
			viewer.setInput(this); // if viewer's input is null, it's content provider is not asked for the contents!
		}
		viewer.refresh();
		updateButtonEnabledStates();	
    }
