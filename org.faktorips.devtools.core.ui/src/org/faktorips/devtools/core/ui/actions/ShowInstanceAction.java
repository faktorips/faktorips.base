...
public class ShowInstanceAction extends IpsAction {
	...
	/**
	 * Use this constructor if you did not already extracted the IIpsElement from the selectionProvider
	 * 
	...
	private void initialize(IIpsElement selectedElement) {
		if (selectedElement instanceof IIpsMetaClass) {
            this.setText(Messages.ShowInstanceAction_nameForTypes);
            this.setDescription(Messages.ShowInstanceAction_descriptionForTypes);
	...
	public void run(IStructuredSelection selection) {
		...
		if (InstanceExplorer.supports(ipsObject)) {
	        try {
		        IViewPart pse = IpsPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().showView(InstanceExplorer.EXTENSION_ID);
