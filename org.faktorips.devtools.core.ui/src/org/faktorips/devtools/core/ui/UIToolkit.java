...
public class UIToolkit {
    ...
    public Combo createCombo(Composite parent, EnumDatatype datatype) {
        ...
        if (datatype.isSupportingNames()) {
            ...
            for (int i = 0; i < ids.length; i++) {
                String formatedText = IpsPlugin.getDefault().getIpsPreferences().getDatatypeFormatter().formatValue(datatype, ids[i]);
	...
	public Combo createCombo(Composite parent, IEnumValueSet enumValueSet,
        ...
        for (int i = 0; i < values.length; i++) {
            if (dataType != null) {
                String formatedText = IpsPlugin.getDefault().getIpsPreferences().getDatatypeFormatter().formatValue(dataType, enumValueSet.getValue(i));
