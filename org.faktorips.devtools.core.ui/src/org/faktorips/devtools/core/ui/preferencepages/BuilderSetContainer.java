...
public class BuilderSetContainer {
    ...
    private void createTableViewer(Composite parent, BuilderSetAdapter adapter) {
        ...
        columns[PROPERTY_DESCRIPTION_COLUMN_INDEX].setLabelProvider(new ColumnLabelProviderWithWrappedTooltip(
                o -> (o instanceof IIpsBuilderSetPropertyDef)
                        ? ((IIpsBuilderSetPropertyDef)o).getDescription()
                        : IpsStringUtils.EMPTY));

        TableUtil.createEditor(tableViewer);
    ...
    private void updateBuilderSet() {
        ...
        TableUtil.updateColumnWidths(tableViewer, columns);
    }
