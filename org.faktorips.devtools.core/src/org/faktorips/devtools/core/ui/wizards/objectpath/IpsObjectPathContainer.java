...
/**
 * Widget creation for referenced IPS projects
 * 
 * @author Roman Grutza
 */
public class IpsObjectPathContainer {
    ...
    public Control createControl(Composite parent) throws CoreException {
        ...
        refProjectsComposite.init(currentIpsProject);
    ...
    // Persists changes made in the "Referenced IPS projects"-Tab 
    private void finishReferencedProjects() {
        if (! refProjectsComposite.isDataChanged()) {
            return;
        }
        
        // remove all IpsProjRefEntries from current IpsObjectPath
        IIpsProject[] oldIpsProjectRefs = ipsObjectPath.getReferencedIpsProjects();
        for (int i = 0; i < oldIpsProjectRefs.length; i++) {
            ipsObjectPath.removeProjectRefEntry(oldIpsProjectRefs[i]);
        }
        
        // add changed IPS project references to IPS object path
        IIpsProject[] newIpsProjects = refProjectsComposite.getReferencedIpsProjects();
        for (int i = 0; i < newIpsProjects.length; i++) {
            ipsObjectPath.newIpsProjectRefEntry(newIpsProjects[i]);
        }
    }
