...
public class MessageCueController {
	...
	/**
	 * Installs or de-installs a visual cue indicating messages related to the
	 * control's content. If the user moves the mouse over the cue image or the control
	 * the messages are displayed in a hover box above the control.
	 * 
	...
	private static final String MESSAGE_CUE_CONTROLLER= MessageCueController.class.getName(); //$NON-NLS-1$
...
//		    }
		...
		void install() {
			...
			String dataIdentifier = ANNOTATION_HANDLER + this.toString();
			Control c= fControl.getParent();
			while (c != null) {
		...
		void uninstall() {
			...
			Control c= fControl.getParent();
			while (c != null) {
				MessageCuePainter cueHandler= (MessageCuePainter) c.getData(ANNOTATION_HANDLER+ this.toString());
				if (cueHandler != null) {
		...
		/**
		 * Shows the hover if the mouse event has taken place on the cue image,
		 * otherwise hides the hower.
		 */
...
//			}
	...
	/**
	 * A single plain HoverController is registered for the content assist control.
	...
	/**
	 * One MessageCuePainter is registered per ancestor control of the content assist control.
