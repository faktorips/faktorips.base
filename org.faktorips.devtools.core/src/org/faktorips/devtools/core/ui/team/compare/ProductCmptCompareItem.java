...
/**
 * Treeitem used to create a tree/structure for representing a product component in the ProductCmptCompareViewer.
 * The IpsSrcFile, the product component, its generations and the relations defined in each generation are
 * represented by ProductCmptCompareItems.
 * Instances of this class are used to compare the contents (and thus structures) of a product component.
 * <p>
 * As product components are displayed in the compareViewer using a single text format, differences between
 * products are displayed as ranges in the text representation.  
 * <p>
 * Each compare item contains a range that defines its position (offset) and length in this text representation.
...
public class ProductCmptCompareItem implements IStreamContentAccessor, IStructureComparator, ITypedElement, IDocumentRange{
    ...
    /**
     * Name of this compareItem. Used as Labels in the compare GUI (DiffNodes, Tree in StructureDiffVIewer).
     */
    private String name;
    private Document document;
        
    /**
     * Creates a ProductCmptCompareItem with the given parent and the given content.
     * If parent is null this ProductCmptCompareItem is marked as a root element,
     * as indicated by the method isRoot().
     */
     ...
     * Returns null.
     * <p>
     * Used for by the compare-engine (Differencer) to guess the filecontents in order to
     * open the appropriate ContentMergeViewer. In case of product components he ContentMergeViewer
     * is identified using the file extension (ipsproduct).
     * <p>
     * For compare operations the compare class retrieves the contents of this item via the StructureCreator.
     * @see ProductCmptCompareItemCreator#getContents(Object, boolean)
     * {@inheritDoc}
     */
    public InputStream getContents() throws CoreException {
//        String contents= getContentString(true);
        ...
        if(hasChildren()){
            ...
            if(ipsElement instanceof IIpsObjectGeneration){
                ...
                if(!ces.isEmpty()){
                    ...
                    sb.append(TAB).append(attrString).append(COLON_BLANK).append(NEWLINE);
                    currentLength += TAB.length()+attrString.length()+COLON_BLANK.length()+NEWLINE.length();
                ...
                if(!rels.isEmpty()){
                    ...
                    sb.append(TAB).append(relString).append(COLON_BLANK).append(NEWLINE);
                    currentLength += TAB.length()+relString.length()+COLON_BLANK.length()+NEWLINE.length(); 
...
//    }
     ...
     * name and other attributes of the wrapped IpsElement. Childcontents are not included.
     * <p>
    ...
    private String initContentString(){
        ...
        if(ipsElement!=null){
            if(ipsElement instanceof IProductCmptRelation){
                ...
                sb.append(Messages.ProductCmptCompareItem_Generation).append(COLON_BLANK);
                sb.append(gen.getName()).append(NEWLINE);
                sb.append(TAB).append(org.faktorips.devtools.core.ui.editors.productcmpt.Messages.GenerationEditDialog_labelValidFrom).append(BLANK);
                ...
                sb.append(TAB).append(org.faktorips.devtools.core.ui.editors.productcmpt.Messages.ProductAttributesSection_attribute).append(COLON_BLANK).append(NEWLINE);
                sb.append(TAB).append(TAB).append(org.faktorips.devtools.core.ui.editors.productcmpt.Messages.ProductAttributesSection_template).append(COLON_BLANK);
                sb.append(QUOTE).append(product.getPolicyCmptType()).append(QUOTE).append(NEWLINE);
                sb.append(TAB).append(TAB).append(org.faktorips.devtools.core.ui.editors.productcmpt.Messages.ProductAttributesSection_labelRuntimeId).append(COLON_BLANK);
    ...
    /**
     * Returns a String representation of this ProductCmptCompareItem including its attributes
     * but <em>not</em> including its children. For a content string including children
    ...
    private String initName(){
        ...
        if(ipsElement!=null){
            if(ipsElement instanceof IProductCmptRelation){
                ...
                IProductCmpt product= (IProductCmpt) ipsElement;
                sb.append(org.faktorips.devtools.core.ui.editors.productcmpt.Messages.ProductCmptEditor_productComponent).append(QUOTE).append(product.getName()).append(QUOTE);
            }else if(ipsElement instanceof IIpsSrcFile){
                IIpsSrcFile srcFile= (IIpsSrcFile) ipsElement;
                IFile file= srcFile.getCorrespondingFile();
                if(file!=null){
                    sb.append(Messages.ProductCmptCompareItem_SourceFile).append(COLON_BLANK).append(QUOTE).append(file.getName()).append(QUOTE);
                }else{
                    sb.append(Messages.ProductCmptCompareItem_SourceFile);
                }
            }
    ...
    /**
     * Returns true if this <code>ProductCmptCompareItem</code> is the root of a tree, false otherwise.
     * A ProductCmptCompareItem is a root if it does not have a parent.
     * @return
     */
    ...
    /**
     * Returns the document the tree of all CompareItems represents. This Document is used when displaying  
     * a files contents in the TextMergeViewer.
     * <p>
     * For compare operations the compare class retrieves the contents of this item via the StructureCreator.
     * @see ProductCmptCompareItemCreator#getContents(Object, boolean)
     */
