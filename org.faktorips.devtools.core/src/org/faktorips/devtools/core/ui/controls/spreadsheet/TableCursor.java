...
public class TableCursor extends Canvas 
{
	...
	/**
	 * IMethod to disable the column listeners. This should be called
	 * before the columns are allocated or changed. 
	 */
	...
	/**
	 * IMethod to enable the column listeners. This should be called
	 * after the columns are allocated or changed. 
	 */
	...
	/**
	 * Paint the cell of the table cursor.
	 * The <code>paint</code> method is different from the original table
	 * cursor.  
	 * 
	...
	void paint(Event event)
	{
		...
		int textY =
			y + (int) (((float) size.y - (float) gc.getFontMetrics().getHeight()) / 2.0 + 0.5);
