...
public class SpreadsheetControl extends Composite {
    ...
    public SpreadsheetControl(
        ...
        this.columnInfos = contentProvider.getColumnInfos();
    ...
    private void initPopupMenu() {
		...
		item.setImage(IpsPlugin.getDefault().getImage("Delete.gif")); //$NON-NLS-1$
		...
		item.setImage(IpsPlugin.getDefault().getImage("Clear.gif")); //$NON-NLS-1$
	...
	/**
	 * Creates the TableColumns for the table control. Also used
	 * to reflect structural changes on the table on the GUI by adding or
	 * removing table columns in the table control.
	 */
	 ...
	 * @param columnInfos, an array of column infos to create editors
	 *        for.
	 */
	...
	/**
	 * Create the viewer object. The viewer object is created once at the first
	 * call to createTableViewer. Each additional call will reinitalize the
	 * viewer to reflect the changes of the table.
	 * 
	...
	/**
	 * Delete the currently selected row. If no row is selected
	 * this method does simply nothing.
	 * The method is used by the menu handler.
	 */
	...
	/**
	 * Sets the currently selected cell to null. This method is used by
	 * the menu handler.
	 */
	...
	/**
	 * This method is called whenever a selection is made. If 
	 * a new row has been added to the end of the table. The row
	 * is checked for empty values. If a row consists only of empty
	 * values, the row is removed again.  
	 */
	 ...
	 * 
	 * @param forward, if true, the cursor is moved forward, otherwise
	 *        it is set to the previous cell. 
	 */
    ...
    private class LabelProvider implements ITableLabelProvider {
        ...
        public Image getColumnImage(Object element, int columnIndex) {
            if (isSizingColumn(columnIndex)) {
                return SpreadsheetControl.this.getRowHeightImage();
            } else {
	...
	/**
	 * The traverse listener processes the key events for cell editors and 
	 * table cursors.
	 * Its main purpose is to provide handling of the tab key.  
	 */
