...
public abstract class JavaSourceFileBuilder implements IIpsArtefactBuilder {
     ...
     * merging capabilities are activated. It indicates that a piece of code was generated in the
     * first place and will not be overridden by the generator by further builds.
     */
     ...
     * 
     * @param packageStructure the package information for the generated java source file and for
     *            other generated java classes within this package structure. Cannot be null.
     ...
     *            exception is thrown
     */
    public JavaSourceFileBuilder(IJavaPackageStructure packageStructure, String kindId,
    ...
    /**
     * Returns the qualified class name for the class definition contained in the java source file
     * that is generated by this builder.
     * 
     * @param object the IpsObject this builder is registered for
    ...
    /**
     * Returns the unqualified class name for the class definition contained in java source file
     * that is generated by this builder.
     * 
     * @param object the IpsObject this builder is registered for
     * @return the qualified class name
    ...
    public String getLocalizedText(String key) {
        ...
        return getLocalizedStringSet().getString(key,
            getIpsObject().getIpsProject().getGeneratedJavaSourcecodeDocumentationLanguage());
    }
    ...
    public void build(IIpsSrcFile ipsSrcFile) throws CoreException {
        ...
        if (isMergeEnabled() && !newFileCreated) {
            ...
            try {
                ...
                is = merge(javaFileContents, newContents, charset);
                javaFile.setContents(is, true, false, null);
                return;
            } finally {
    ...
    public void delete(IIpsSrcFile ipsSrcFile) throws CoreException {
        ...
        IFolder destination = ipsSrcFile.getIpsPackageFragment().getRoot().getArtefactDestination();
        if (file.exists()) {
    ...
    private String format(String content) {
        ...
        TextEdit edit = formatter.format(CodeFormatter.K_COMPILATION_UNIT, content, 0, content
                .length(), 0, StringUtil.getSystemLineSeparator());
    ...
    public IFile getJavaFile(IIpsSrcFile ipsSrcFile) throws CoreException {
        IFolder destinationFolder = ipsSrcFile.getIpsPackageFragment().getRoot()
    ...
    private InputStream merge(InputStream oldContent, InputStream newContent, String charset) {
        JControlModel model = null;
        InputStream is = null;
        try {
            StringBuffer mergeFile = new StringBuffer();
            mergeFile.append('/').append(
                JavaSourceFileBuilder.class.getPackage().getName().replace('.', '/')).append(
                "/merge.xml");
            is = (InputStream)Platform.getBundle(IpsPlugin.PLUGIN_ID).getResource(
                mergeFile.toString()).getContent();
        ...
        merger.setControlModel(model);
        merger.setSourceCompilationUnit(merger.createCompilationUnitForInputStream(newContent));
        merger.setTargetCompilationUnit(merger.createCompilationUnitForInputStream(oldContent));
        merger.merge();
        try {
            String targetContents = merger.getTargetCompilationUnitContents();

            if (targetContents == null) {
                return new ByteArrayInputStream("".getBytes(charset));
            }
            return new ByteArrayInputStream(targetContents.getBytes(charset));
    ...
    private ByteArrayInputStream transform(IIpsSrcFile ipsSrcFile, String content)
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR, "org.faktorips.std.builder",
                    IStatus.OK, "The charset is not supported: " + charset, e));
        }
    }
