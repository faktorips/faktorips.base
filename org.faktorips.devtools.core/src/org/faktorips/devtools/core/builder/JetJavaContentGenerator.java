 ...
 * at the right position within the code the markImportLocation() method has to be used. See the method
 * description to used it correctly. At the end of a JET-template the addImports() method needs to be
 * called to actually add the import declarations to the source content. 
 * 
 * @author Peter Erzberger
 */
public abstract class JetJavaContentGenerator {

    private ImportDeclaration importDeclartion;
     ...
     * 
     * 
     * @param ipsSrcFile the IpsObject that is provided to this method by the JetJavaSoureFileBuilder
     *            during the build routine
     ...
     */
    public abstract String generate(IIpsSrcFile ipsSrcFile)
    ...
    public JetJavaContentGenerator() {
        super();
        importDeclartion = new ImportDeclaration();
    }
    ...
    protected void addImports(StringBuffer buffer) {
        ...
        for (Iterator it = importDeclartion.iterator(); it.hasNext();) {
            ...
            if (it.hasNext()) {
                importBuffer.append(StringUtil.getSystemLineSeparator());
            }
        }
