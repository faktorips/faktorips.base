 ...
 * <p>
 * This cache caches the ips source file contents and, as the ips source file content helds a reference
 * to the ips object stored in the file, also ips objects. The cache is designed to allow the garbage
 * collector to reclaim the cached objects. This is archived by using soft references.  
 * <p>
 * The following fact has to be considered in the implementation:<p>
 * IpsSrcFileContent objects are an internal implementation detail of the model. They are not visible to 
 * the client (like the UI and the builder) using the published API. In fact the only object holding a 
 * reference to them, is this cache. On the other hand, clients work directly with ips objects and
 * thus are allowed to hold (strong) references to them. So this cache can't hold soft references to 
 * ips source file contents objects only. What would happen (and actually this has been a bug) is, that
 * the garbage collector removes an ips source file content object held in the cache, while a client holds
 * a reference to the ips object referenced by the content object. The garbage collector is allowed to
 * remove the contents object as this cache is the only object holding a reference to it and this reference
 * is a soft one. So what happens is that the next time another client (not the one already holding a reference
 * to the ips object), the ips source file content is not found in the cache. Thus a new one including an ips 
 * object is created and we end up having two instances for the same logical ips object (which is pretty bad).  
 * <p>
 * 
 * @see SoftReference
 * 
...
public class IpsSrcFileContentsCache {
     ...
     * this size of entries will be stored as hard reference (order of last access) 
     * and never deleted by the garbage collector
     */
     ...
     * 
     * @return previous object associated with specified key, or <tt>null</tt>
