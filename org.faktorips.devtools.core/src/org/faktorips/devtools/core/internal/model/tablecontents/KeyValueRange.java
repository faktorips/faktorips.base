...
public class KeyValueRange extends AbstractKeyValue implements Comparable<KeyValueRange> {
    ...
    // the 'to'-value will be evaluated (using the range and row object) if necessary (see evalToValue)
    private String valueFrom;
    
    private KeyValueRange(ITableStructure structure, ValueDatatype[] datatypes, IUniqueKey uniqueKey, ColumnRange columnRange, Row row) {
    ...
    /**
     * Creates a new key value range object of an a given unique key for a given row. The unique key must contain at least one column range.
     * The key value range object will be created using the given column range.
     */
    public static KeyValueRange createKeyValue(ITableStructure structure, ValueDatatype[] datatypes, IUniqueKey uniqueKey, Row row, ColumnRange columnRange){
    ...
    /**
     * Returns <code>true</code> if the 'from'- and 'to'-value are parsable.
     * For instance the values are not parsable if the table contents row is erroneous. 
     * 
    ...
    /**
     * Returns the stored 'from'-value of the column range. Note that the from value could be another 
     * than the current column value in the row.
     * 
     * @see #isValid()
     */    
     ...
     * changed the evalValueFrom() method can return a different value as getKeyValue() method.
     * (see also #isValid())
     */
    ...
    public static boolean isRangeCollision(ITableStructure tableStructure,
        ...
        if (fromCompareToOtherFrom == 0 || 
            fromCompareToOtherTo == 0 || 
            toCompareToOtherFrom == 0 || 
