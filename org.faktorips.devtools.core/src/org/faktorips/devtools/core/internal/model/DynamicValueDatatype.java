...
/**
 * A dynamic value datatype is a generic value datatype where the Java class represented
 * by the datatype is defined by it's qualified class name. The class is resolved, when one of
 * the datatype's method like is isParsable() is called, which needs a static method of the underlying
 * Java class. If the Java class' source belongs to the IpsProject/JavaProject, than this class 
 * (the bytecode) might exist for some time and than be unavailable or incomplete, e.g. if the class
 * has compile errors. 
 * 
 * @author Jan Ortmann
 */
public class DynamicValueDatatype extends GenericValueDatatype {
    ...
    public final static DynamicValueDatatype createFromXml(IIpsProject ipsProject, Element element) {
        ...
        if (element.hasAttribute("specialNullValue")) {
            datatype.setSpecialNullValue(element.getAttribute("specialNullValue"));
        }
    ...
    public Class getAdaptedClass() {
        if (adaptedClass==null) {
            try {
                adaptedClass = getProjectClassloader(ipsProject.getJavaProject()).loadClass(className);
                // get the URL from the resource the class was loaded from
                URL url = adaptedClass.getResource("/" + adaptedClass.getName().replace('.', '/') + ".class");
                ...
                // ok to squeeze, the datatype remains invalid as long as the class can't be loaded.
            }
        }
        return adaptedClass;
    }
    ...
    private void startListeningToClassContainerChanges(String file) {
        ...
        IPath projectRelativePathToFile = absolutPathToFile.removeFirstSegments(absolutPathToProject.segmentCount());
        classContainerFile = ipsProject.getProject().getFullPath().append(projectRelativePathToFile);
        changeListener = new ChangeListener();
        ipsProject.getProject().getWorkspace().addResourceChangeListener(changeListener, IResourceChangeEvent.POST_CHANGE | IResourceChangeEvent.PRE_BUILD);
    }
    ...
    private class ChangeListener implements IResourceChangeListener {
        ...
        public void resourceChanged(IResourceChangeEvent event) {
            ...
            IResourceDelta delta = event.getDelta().findMember(classContainerFile);
            if (delta==null) {
                return;
            }
            ipsProject.getProject().getWorkspace().removeResourceChangeListener(changeListener);
            changeListener = null;
            removeCachedData();        
        }
    }
