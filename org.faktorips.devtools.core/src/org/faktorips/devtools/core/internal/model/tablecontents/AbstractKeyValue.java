...
/**
 * Abstract basis class of all kind of key values (e.g. column 'only' key values or 'range' key values).
 * A key value is an object which stores the value of an unique key of a specific table row.
 */
public abstract class AbstractKeyValue {
    ...
    /*
     * Helper method, returns all non two column range key items: column key items or column range key items with type ONE_COLUMN_RANGE_FROM
     * or ONE_COLUMN_RANGE_TO
     */
    protected static List<IKeyItem> getNonTwoColumnRangeKeyItems(IUniqueKey uniqueKey) {
        ...
        for (int i = 0; i < keyItems.length; i++) {
            if (keyItems[i] instanceof IColumnRange){
                if (ColumnRangeType.TWO_COLUMN_RANGE.equals(((IColumnRange)keyItems[i]).getColumnRangeType())){
                    continue;
                }
            }
            result.add(keyItems[i]);
        }
        return result;
    }
     ...
     * afterwards this row has changed the column - which is used within an unique key - then the key
     * value will be invalid (obsolete).
     */
