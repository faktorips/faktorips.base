 ...
 * 
 * This source code is available under the terms of the AGPL Affero General License version 3.
 * 
...
public interface IIpsPackageFragment extends IIpsElement {
    ...
    /**
     * Returns the package fragment which contains this one or null if this one is the default-package.
     */
    ...
    /**
     * Returns the {@link Comparator} to be used when displaying this package fragment's contents in an
     * ordered fashion.
     */
    Comparator<IIpsElement> getChildOrderComparator();

    /**
     * Returns all <code>IResource</code>s that do not correspond to <code>IIpsPackageFragment</code>s
     * contained in this PackageFragment. Returns an empty array if no such resources are found.
     * 
    ...
    /**
     * Returns a handle to the IPS source file with the given name. If the provided name doesn't have a
     * file extension that fits to an IpsSrcFile <code>null</code> will be returned.
     */
    ...
    /**
     * Creates a new IpsSrcFile based on a given template. The filename is constructed by appending the
     * type specific file extension to the given object name (separated by a dot). The content of the
     * IpsSrcFile is copied from the given template. If the template is a ITimedIpsObject, only the
     * generation that is valid at the given date is copied. If no generation is valid at the given date
     * because the date lies before any valid generation of the given template, the first generation of
     ...
     * @param newDate the date the new generation is effective from, if the template is an instance of
     *            {@link ITimedIpsObject}
     * @param force a flag controlling how to deal with resources that are not in sync with the local
     *            file system
     * @param monitor the given progress monitor
     * 
     ...
     *             </ul>
     * 
     * @deprecated since 3.9 because the dates specified here are only used in case that the template
     *             object is an {@link ITimedIpsObject}. Please use
     ...
     *             necessary to call
     *             {@link ITimedIpsObject#retainOnlyGeneration(GregorianCalendar, GregorianCalendar)} if
     *             the copied object is an {@link ITimedIpsObject}.
     */
    @Deprecated
    IIpsSrcFile createIpsFileFromTemplate(String name,
            IIpsObject template,
    ...
    /**
     * Returns an {@link IIpsPackageFragment} that is located in this package and is identified by the
     * name of this package extended with the sub package name. The name of the new package is
     * 'name.subPackageName'.
     * <p>
     * In contrast to {@link #createSubPackage(String, boolean, IProgressMonitor)} this method does not
     * create the folders for the package if they do not exists
     * 
     * @param subPackageFragmentName The last segment of the name of the sub package fragment
     * @return The {@link IIpsPackageFragment} that is located under this package and have the specified
     *         name segment appended.
     */
    ...
    /**
     * Returns <code>true</code> if this IIpsPackageFragement has at least one IIpsPackageFragement as a
     * child.
     */
