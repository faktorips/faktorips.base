...
/**
 * Interface for the extension point org.faktorips.plugin.artefactbuilderset of this plugin. Only
 * one implementation of this interface can be registered for this extension point. If more than one
 * extensions are declared only the first one will be registed and the others will be ignored. An
 * IpsArtefactBuilderSet collects a list of IpsArtefactBuilders and makes them available to the
 * build system.
 * 
...
public interface IIpsArtefactBuilderSet extends IJavaPackageStructure {
    ...
    /**
     * Returns <code>true</code> if the builder set supports an formula language
     * identifierResolver.
     */
     ...
     * 
     * @return <code>true</code> if this builder set contains an builder which requires to be
     *         called for the aggregate root object if any child (regardless of the relations depth)
     *         has been modified. Only composite-relations are allowed for this dependency scan.
     *         <code>false</code> if only builder are contained which need only a dependency scan
     *         of depth one.
     */
     ...
     * called before the <code>beforeBuildProcess(IIpsProject, int)</code> method of the
     * registered IpsArtefactBuilders will be called.
     * 
    ...
    /**
     * This method is called when the build process is finished for this builder set. This method is
     * called after the <code>afterBuildProcess(IIpsProject, int)</code> methods on the registered
     * IpsArtefactBuilders were called.
     * 
     * @param buildKind One of the build kinds defined in
     *            <code>org.eclipse.core.resources.IncrementalProjectBuilder</code>
     * 
