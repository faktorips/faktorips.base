...
public interface ITestRule extends ITestObject {
    ...
    /**
     * Validation message code to indicate that the validation rule is inside the policy cmpts which are related by
     * the test policy cmpt type parameters inside the test case type.
     */
     ...
     */
    public final static String MSGCODE_TEST_RULE_PARAM_NOT_FOUND = MSGCODE_PREFIX
        + "TestRuleParamNotFound"; //$NON-NLS-1$
    ...
    /**
     * Sets if the validation rule is expected to be violated or not expected to
     * be violated.
     * 
