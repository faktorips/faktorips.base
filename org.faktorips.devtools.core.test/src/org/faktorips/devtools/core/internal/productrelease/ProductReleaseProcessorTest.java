...
public class ProductReleaseProcessorTest extends AbstractIpsPluginTest {
    ...
    protected void setUp() throws Exception {
        ...
        // productReleaseProcessor = spy(new ProductReleaseProcessor(ipsProject,
        // observableMessages));
        //
    ...
    // assertEquals(Messages.ReleaseAndDeploymentOperation_exception_noDeploymentExtension,
    // messageList.getMessage(0)
    // .getText());
    //
    ...
    // assertEquals(Messages.ProductReleaseProcessor_error_custom_validation_failed,
    // e.getMessage());
    // }
    //
    // releaseAndDeploymentOperation = mock(IReleaseAndDeploymentOperation.class);
    // when(productReleaseProcessor.getReleaseAndDeploymentOperation()).thenReturn(releaseAndDeploymentOperation);
    // when(releaseAndDeploymentOperation.customReleaseSettings(any(IIpsProject.class),
    ...
    //
    // verify(releaseAndDeploymentOperation).customReleaseSettings(eq(ipsProject),
    // any(IProgressMonitor.class));
    //
    // verify(releaseAndDeploymentOperation).buildReleaseAndDeployment(eq(ipsProject), eq("abc"),
    // eq(targetSystems),
    // any(IProgressMonitor.class));
    //
